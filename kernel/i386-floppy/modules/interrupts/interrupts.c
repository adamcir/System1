...
static void idt_set_gate(uint8_t vector, uint32_t handler_addr, uint16_t selector, uint8_t type_attr) {
    g_idt[vector].offset_low = (uint16_t)(handler_addr & 0xFFFFu);
    g_idt[vector].selector = selector;
    g_idt[vector].zero = 0;
    g_idt[vector].type_attr = type_attr;
    g_idt[vector].offset_high = (uint16_t)((handler_addr >> 16) & 0xFFFFu);
}
...
void interrupts_init(void) {
    ...
    for (i = 0; i < 48; i++) {
        idt_set_gate(i, (uint32_t)(uintptr_t)g_isr_stub_table[i], code_selector, 0x8E);
    }
